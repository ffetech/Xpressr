...
{
    public class RptDataSetRange : RptRange
    {
        #region Constructors

        public RptDataSetRange(RptRange parent)
            : base(parent)
        {
        }
        ...
        {
            if (Limit > 0 && CurrentRecordCount >= Limit)
                return false;
        ...
        {
            if (!dataSet.TryGetDataSet(Name, out dataSet))
            {
                if (Fallback != null)
                {
                ...
                throw new RptDataException($"Unknown Dataset '{Name}'");
            }
            ...
            while (dataSet.CurrentRecord != null && dataSet.CurrentRecord != bookmark)
            {
                bookmark = dataSet.CurrentRecord;
