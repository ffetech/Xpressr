...
{
    public class RptChartMeasureAxis : RptChartAxis
    {
        #region Constructors

        public RptChartMeasureAxis(RptChartElement chart)
            : base(chart)
        {
            Size = 50;
        }
        ...
        {
            output.AppendLine(string.Format("{0}var measureAxis = chart.addMeasureAxis(\"{1}\", \"{2}\");", indent, axis, "Value"));
            AppendFormat(output, "measureAxis", indent);
        }
