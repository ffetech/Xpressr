...
{
    internal class RptTemplateReader : ExpressionVisitor, IRptExpressionVisitor
    {
        #region Nested Types

        private class EndExpressionTarget : IRptExpressionTarget
        {
            #region Constructors

            public EndExpressionTarget(RptRange target)
            {
                Target = target;
            }
            ...
            {
                throw new RptTemplateException("End expression have no properties");
            }
        ...
        private readonly Stack<RptRange> composite = new Stack<RptRange>();

        private RptRange currentRange;

        #endregion

        #region Constructors

        private RptTemplateReader(RptDocument document)
        {
            composite.Push(document);
            currentRange = document;

            RegisterExpressionReader(new EmbraceExpressionReader("Comment", "{*", "*}"));
            RegisterExpressionReader(new RptMarkupExpressionReader("Default", this));
        }
        ...
        Type IRptExpressionVisitor.GetElementType(string name)
        {
            return currentRange.GetElementType(name);
        }

        IRptExpressionTarget IRptExpressionVisitor.CreateElementTarget(Type elementType)
        {
            return currentRange.CreateChild(elementType);
        }

        IRptExpressionTarget IRptExpressionVisitor.CreateElementEndTarget(Type elementType)
        {
            if (currentRange.GetType() != elementType)
                throw new RptTemplateException("End of range don't fits to the range start");
        ...
        public static void Read(RptDocument document, string template)
        {
            new RptTemplateReader(document).Visit(template);
        }
        ...
        {
            currentRange.AddChild(new RptContentElement(currentRange, text));
        }
        ...
        protected override void VisitExpression(IExpression expression)
        {
            ...
            IRptExpressionTarget target = ((RptMarkupExpression)expression).Target;

            if (target is RptElement)
            {
                currentRange.AddChild((RptElement)target);

                RptRange newRange = target as RptRange;
                if (newRange != null)
                {
                    composite.Push((RptRange)target);
